...
export async function onRequestPost({request,env}){if(!ok(request,env))return Response.json({error:'Unauthorized'},{status:401});const db=dbOf(env);let b;try{b=await request.json()}catch{return Response.json({error:'Invalid request'},{status:400})};
  if(clean(b.action,40)==='remove'){const id=Number(b.id||0);if(!id)return Response.json({error:'Assignment ID required.'},{status:400});const row=await db.prepare(`SELECT id,participant_count_locked,final_commission,course_status FROM coach_course_assignments WHERE id=?`).bind(id).first();if(!row)return Response.json({error:'Assignment not found.'},{status:404});if(Number(row.participant_count_locked)||row.final_commission!=null||['Conducted','Cancelled'].includes(String(row.course_status||'')))return Response.json({error:'Closed/finalized course assignments cannot be removed.'},{status:409});await db.prepare(`DELETE FROM coach_course_assignments WHERE id=?`).bind(id).run();return Response.json({ok:true,removed_id:id});}
  const coachId=Number(b.coach_id||0),productId=Number(b.product_id||0);if(!coachId||!productId)return Response.json({error:'Coach and course are required.'},{status:400});const c=await db.prepare(`SELECT id,status FROM coaches WHERE id=?`).bind(coachId).first();if(!c||c.status!=='Approved')return Response.json({error:'Only Approved coaches may be assigned.'},{status:400});
