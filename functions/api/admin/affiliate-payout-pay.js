...
export async function onRequestPost({request,env}){
  ...
  try{
    ...
    const payoutId=Number(b.payout_id||0);
    const paymentReference=clean(b.payment_reference,160);
    const paymentDate=clean(b.payment_date,32);

    if(!payoutId || !paymentReference || !paymentDate){
      return Response.json(
        {
          error:
            'payout_id, payment_reference and payment_date are required.'
        },
        {status:400}
      );
    }
    ...
    // D1 batch is atomic: if one statement fails, the whole batch rolls back.
    ...
    console.error('affiliate payout pay',e);
    return Response.json(
      {
        error:'Unable to record payout payment.',
